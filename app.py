...
def get_frames_from_video(video_input, video_state):
    ...
    video_state = {
        "user_name":
            user_name,
        "video_name":
            os.path.split(video_path)[-1],
        "origin_images":
            frames,
        "painted_images":
            frames.copy(),
        "masks": [np.zeros(
            (frames[0].shape[0], frames[0].shape[1]), np.uint8)] * len(frames),
...
def select_template(image_selection_slider, video_state, interactive_state):
    ...
    return video_state["painted_images"][
        image_selection_slider], video_state, interactive_state, operation_log
...
def get_end_number(track_pause_number_slider, video_state, interactive_state):
    ...
    return video_state["painted_images"][
        track_pause_number_slider], interactive_state, operation_log
...
def sam_refine(video_state, point_prompt, click_state, interactive_state,
               evt: gr.SelectData):
    """
    Args:
    ...
    video_state["painted_images"][
        video_state["select_frame_number"]] = painted_image
...
def add_multi_mask(video_state, interactive_state, mask_dropdown):
    try:
        ...
        select_frame, run_status = show_mask(video_state, interactive_state,
                                             mask_dropdown)
        ...
        operation_log = [("Please click the left image to generate mask.",
                          "Error"), ("", "")]
    return interactive_state, gr.update(
...
def show_mask(video_state, interactive_state, mask_dropdown):
    ...
    for i in range(len(mask_dropdown)):
        ...
        select_frame = mask_painter(select_frame,
                                    mask.astype('uint8'),
                                    mask_color=mask_number + 2)
...
def vos_tracking_video(video_state, interactive_state, mask_dropdown):
    ...
    if interactive_state["track_end_number"]:
        following_frames = video_state["origin_images"][video_state[
            "select_frame_number"]:interactive_state["track_end_number"]]
    else:
    ...
    if interactive_state["multi_mask"]["masks"]:
        ...
        template_mask = interactive_state["multi_mask"]["masks"][
            int(mask_dropdown[0].split("_")[1]) - 1] * (int(
    ...
    masks, logits, painted_images = model.generator(images=following_frames,
    ...
    if interactive_state["track_end_number"]:
        video_state["masks"][video_state["select_frame_number"]:
                             interactive_state["track_end_number"]] = masks
        video_state["logits"][video_state["select_frame_number"]:
                              interactive_state["track_end_number"]] = logits
        video_state["painted_images"][
            video_state["select_frame_number"]:
            interactive_state["track_end_number"]] = painted_images
    else:
        video_state["masks"][video_state["select_frame_number"]:] = masks
        video_state["logits"][video_state["select_frame_number"]:] = logits
        video_state["painted_images"][
            video_state["select_frame_number"]:] = painted_images
    ...
    print(
        "For generating this tracking result, inference times: {}, click times: {}, positive: {}, negative: {}"
        .format(
    ...
    if interactive_state["mask_save"]:
        if not os.path.exists('./result/mask/{}'.format(
                video_state["video_name"].split('.')[0])):
            os.makedirs('./result/mask/{}'.format(
                video_state["video_name"].split('.')[0]))
        i = 0
        print("save mask")
...
def inpaint_video(video_state, interactive_state, mask_dropdown):
    ...
    try:
        inpainted_frames = model.baseinpainter.inpaint(
            frames, inpaint_masks,
            ratio=interactive_state["resize_ratio"])  # numpy array, T, H, W, 3
    except:
        operation_log = [(
...
xmem_checkpoint = download_checkpoint(xmem_checkpoint_url, folder,
                                      xmem_checkpoint)
...
model = TrackingAnything(SAM_checkpoint, xmem_checkpoint, e2fgvi_checkpoint,
                         args)


def process_video(input_video_path):
    # Extract frames from the input video
    video_state, _, _, _, _, _, _, _, _, _, _, _, _, _, _ = get_frames_from_video(
        input_video_path, {})

    # Initialize the interactive_state dictionary
    interactive_state = {
    ...
    # # Paint masks on the frames
    # for i in range(len(video_state["origin_images"])):
    #     video_state["painted_images"][i] = mask_painter(video_state["origin_images"][i], video_state["masks"][i])

    # # Generate output video
    # output_video_path = generate_video_from_frames(video_state["painted_images"], output_path="./result/track/{}".format(video_state["video_name"]), fps=video_state["fps"])

    # return output_video_path


input_video_path = "/home/jovyan/workspace/SMD/VIS_Onshore/VIS_Onshore_vids/MVI_1470_VIS.avi"
